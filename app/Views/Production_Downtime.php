...
function findPart(production_shift_date){
  //alert(production_shift_date);
  // alert();
  $.ajax({
    url:"<?php echo base_url('Home/findPartTool'); ?>",
    method:"post",
    data:{
    ...
    success:function(production_shift){
      ...
      // console.log(production_shift.length);
  ...
  // setTimeout(alert(part_name_tooltip), 3000)
  //alert(part_name_tooltip);
}
...
$(document).on('change','#RejectShift',function(){
  ...
  if(machine_name !="select" && shift_date !="" && shift !="select"){
    $('.split_input').empty();
    getPartRefTooltip(machine_name,shift_date,shift);
    getDownTimeGraph();
  }
  
});

function getPartRefTooltip(machine_name,shift_date,shift){
  // alert(machine_name);
  $.ajax({
    url:"<?php echo base_url('Home/findToolTipPart'); ?>",
    method:"post",
    data:{
    ...
    success:function(production_shift){
      var partName ="";
      production_shift.forEach(function(item){
        var part =item.part_name;
        if (item.part_id == null) {
          var k=0;
          // console.log(part_name_tooltip);
          part_name_tooltip.forEach(function(val){
            if (item.start_time <= val.time) {
              part = val.part_name;
          ...
          if (k==0) {
            // console.log(part_name_tooltip);
            partName = part_name_tooltip[(part_name_tooltip.length-1)].part_name;
            part = partName;
          }
        }
        var tmp ={
          "machine_event_id":item.machine_event_id,
          "part_name":part,
          "start_time":item.start_time,
        };
        machine_Data.push(tmp);
        // console.log(part);
      });
    },
    error:function(err){
...
$(document).on("click", ".deleteRec", function(){
      $('#DeleteSPlit').modal('hide');
        ...
        if (indexRef == 1) {  
          var tmp = (parseFloat(data_array[0]).toFixed(1)).split(".");
    ...
    $(document).on("change", ".DownTool", function(){
      ...
      $('.DownPart:eq('+index_value+')').append(elements);
   ...
   function drawGraph(start,svalue,end,machineEventRef,notes,reason=null,part=null,tool=null,splitId,last_updated_by,last_updated_on){
    ...
    function getDownTimeGraph(){
      ...
      //   getDownTimeGraph();
      // }
      // alert(machine_id+" "+shift_date+" "+s[0]);
           ...
           data:{
            ...
            shift:s[0],
                ...
                response['shift']['shift'].forEach(function(item){
                  ...
                  // alert(s[0]);
                  if (x[0] == s[0]) {
                    $('.startTimeVal').html(item.start_time);
                    $('.endTimeVal').html(item.end_time);
                ...
                // alert(response['machineData'].length);
                ...
                $.each(response['machineData'],function(key,model){
                  ...
                  var duration = model.split_duration;
                  ...
                  if (sh == "00") {
                   var sv = parseInt(24*60*60)+parseInt(sm*60)+parseInt(ss); 
                  }
                  else{
                  ...
                  if (eh == "00") {
                    var ev = parseInt(24*60*60)+parseInt(em*60)+parseInt(es);
                  }
                  else{
                  ...
                  var part_Name_Val="";
                  machine_Data.forEach(function(x){
                    if (x.machine_event_id == model.machine_event_id) {
                      part_Name_Val = x.part_name;
                    }
                  });
                  // console.log(part_Name_Val);
                  ...
                  if (sv != ev) {
                    ...
                    // alert(partName);
                    graph_Data.push({name:"No Data",data:[(d).toFixed(2)],color:colordemo,start:stimeData,end:model.start_time,machineEvent:machineEvent,down_notes:model.notes,machine_Name:machine_Name,part_Name:part_Name_Val,duration:duration});
                  }
                  ...
                  sm = te[1];
                  ss = te[2];
                  graph_Data.push({name:model.event,data:[model.duration],color:colordemo,start:model.start_time,end:model.end_time,machineEvent:machineEvent,down_notes:model.notes,machine_Name:machine_Name,part_Name:part_Name_Val,duration:duration});
                  ...
                  if (key == (response['machineData'].length -1)) {
                    ...
                    if (sv != ev) {
                      ...
                      colordemo = color_bar("No Data",0);
                      graph_Data.push({name:"No Data",data:[(d).toFixed(2)],color:colordemo,start:model.end_time,end:stimeData,machineEvent:machineEvent,down_notes:model.notes,machine_Name:machine_Name,part_Name:part_Name_Val,duration:duration});
                    }
                  }  
                });  
                      ...
                      click:function(event, chartContext, config){
                        ...
                        $.ajax({
                          ...
                          success:function(res){
                            ...
                            // alert(sname+" ="+res.length+"- "+res.machine_event_id);
                            
                            if (((res.length > 0) && (sname == "Inactive")) || ((res.length > 0) && (sname == "Machine OFF"))) {
                              
                              var z = 0;
                              res.forEach(function(item){
                                
                                // alert(item.downtime_reason_id);
                                if (item.downtime_reason_id==1) {
                                  machineOFFTotal = machineOFFTotal+1;
                                }
                                else if(item.downtime_reason_id == 4){
                                  UnnamedTotal = UnnamedTotal +1;
                                }
                        ...
                        // alert(machineOFFTotal);
                        $('#machineOFFTotal').text(machineOFFTotal);
                        $('#UnnamedTotal').text(UnnamedTotal);
                  ...
                  tooltip: {
                    custom: function({series, seriesIndex, dataPointIndex, w}) {
                      ...
                      return ('<div class="Tooltip_Container">'+'<div>'
                            +'<p class="paddingm nameHeader">'+machine_Name_Tooltip+'</p>'
...
//Total record(row) count....
    ...
    if (svalue === "") {
      ...
      if (parseInt(svalue)>1) {
        ...
        if (index1 == 0) {
          ...
          data_array[index1] = mathv+"."+parseFloat(fistSec[1]).toFixed(2);
        }
        else{
        ...
        // alert(tmpStime);
        var sh=parseInt(tmpStime[0]);
        var sm=parseInt(tmpStime[1]);
        var ss=parseInt(tmpStime[2]);
        if (parseInt(sh)<=9) {
            sh = "0" + sh;
          }
          if (parseInt(sm)<=9) {
        ...
        for(let i=0;i<l;i++){
          ...
          //total seconds...
          if (i==0) {
            var t = parseInt(xtmpStime)+parseInt(parseInt(firstSecond[0])*60)+parseInt(firstSecond[1]);
          }
          else{
  ...
  if (tempSplit != 1) {
    ...
    // if(data_array.length != 1){
      ...
      //Important code do not remove it.....
      //Hided for checking.......
    ...
    //temporary conversion for shift.......
    var s = shift_id_split.split("");
    //alert("Ref = "+eventRefVal+"Split Ref= "+splitRef+" Data = "+splitDataPass+"shift= "+s[0]);
    $.ajax({
          ...
          if (vale > m) {
            ...
            if (vale >= 1) {
              ...
              if(vale > prev){
                    ...
                    while(j < length){
                      var tmp = ""+("0" + hx).slice(-2)+":"+("0" + mx).slice(-2)+":"+("0" + sec).slice(-2)+"";
                      //2(i)+1 logic
                      data_time[(2*j)]=tmp;
                      ...
                      if (j == (length-1)) {
                        ...
                        tmp = ""+("0" + nhx).slice(-2)+":"+("0" + nmx).slice(-2)+":"+("0" + nsec).slice(-2)+"";
                      }
                      else{
                        tmp = ""+("0" + nhx).slice(-2)+":"+("0" + nmx).slice(-2)+":"+("0" + nsec).slice(-2)+"";
                      }
                ...
                if (index3 == (data_array.length -1)) {
                  ...
                  data_time[j] = ""+("0" + hx).slice(-2)+":"+("0" + mx).slice(-2)+":"+("0" + sec).slice(-2)+"";
                  if (j == (length-1)) {
                    var tmpCalc = data_array[0].split(".");
                    sec= sec+tmpCalc[1];
                    data_time[j+1] = ""+("0" + hx).slice(-2)+":"+("0" + mx).slice(-2)+":"+("0" + sec).slice(-2)+"";
                  }
                  else{
                    data_time[j+1] = ""+("0" + hx).slice(-2)+":"+("0" + mx).slice(-2)+":"+("0" + sec).slice(-2)+"";                  }
                }
          ...
          res_Site.forEach(function(item){
            if (tool == item.tool_id) {
              ...
              if (item.part_id == part) {
                // alert(part);
                elements = elements.add('<option class="DownPartVal" pval="'+item.part_name+'.'+item.part_id+'" tvalue="'+item.tool_id+'" value="'+item.part_id+'" selected="true">'+item.part_name+' -'+item.part_id+'</option>');
              }
              else{
