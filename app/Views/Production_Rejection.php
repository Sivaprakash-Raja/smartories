           ...
           // alert(res);
            ...
            $('.contentMachine').empty();
                res.forEach(function(item){
                    ...
                    //start time change to whole number for display.......
                    if (s[1]>30) {
                        if (i==0) {
                            start = s[0]+":"+"30"+":00";
                        }
                    ...
                    //end time change to whole number for display.......
                    if (e[1]<30) {
                        end = s[0]+":"+"30"+":00";
                        startTime=end;
                    }else{
                        if (i==0) {
                            end = (parseInt(s[0])+parseInt(1))+":"+"00"+":00";
                            startTime=end;
                        }else{
                            var k = startTime.split(":");
                            end = (parseInt(k[0])+parseInt(1))+":"+"00"+":00";
                            startTime=end;
                        }
                    }
...
$(document).on('change','.RejectCount',function(){
   ...
   // var count = document.getElementsByClassName('RejectCount');
    ...
    if ((Number.isInteger(pattern)) && (pattern > 0)) {
        ...
        var array_sum_rcount = myarr.reduce(function(a, b){
...
$(document).on('click','.EditReject_submit',function(){
    ...
    if (parseInt(array_sum_rcount) < parseInt(validate_count_max) ) {
        ...
        for(var i=0;i<reason;i++){
            ...
            if (reason_err == "") {
                document.getElementsByClassName('reject_count_err')[i].textContent = 'Invalid';
                document.getElementsByClassName('reject_reason_err')[i].textContent = 'Invalid';
            }
        }
        // for(var i=0;i<reason;i++){
        //     var reason_err = document.getElementsByClassName('RejectReason')[i].value;
        //     if (reason_err == "") {
        //         document.getElementsByClassName('reject_reason_err')[i].textContent = 'Invalid';
        //     }
        // }
