...
**/
    ...
    public function getDownGraph1($machine_id,$shift_date,$shift){
        ...
        public function splitDownGraph($data,$ref,$splitRef,$last_updated_by,$shift_id,$shift_date){
        $db = \Config\Database::connect();
        ...
        if (sizeof($output) == 0) {
            ...
            //     'end_time' => $data[4],
            // ];
            // if ($builder->insert($val1)) {
            //     if ($builder->insert($val2)) {
            ...
            if ($con->update()) {
                $val2 =[
                    ...
                    'downtime_reason_id' => $rec[0]['downtime_reason_id'],
                    'last_updated_by'=> $last_updated_by,
                ];
                sleep(2);
                if ($con->insert($val2)) {
    ...
    public function updateDownGraph($data,$machineRef,$splitRef,$timeArray,$durationArray,$last_updated_by){
        $db = \Config\Database::connect();
        $builder = $db->table('pdm_downtime_reason_mapping');
        $builder->select('*');
        // $builder->where('Date', $id);
        // $builder->where('Machine_ID', $id);
        // $builder->where('Shift', $id);
        ...
        if (sizeof($output) == 0) {
            ...
            $build = $db->table('pdm_downtime_reason_mapping');
            $j=0;
            for ($i = 0; $i <= (sizeof($durationArray)-1); $i++) {
                if($i == $splitRef){
                    ...
                    $build->set('tool_id',$data[2]);
                    $build->set('part_id', $data[3]);
                    $build->set('last_updated_by',$last_updated_by);
                    $build->where('split_id', $i);
                    $build->where('machine_event_id', $machineRef);
                    if ($build->update()) {
                        $j=$j+1;
                    }
    ...
    public function deleteSPlit($dataVal,$machineRef,$splitRef,$start,$end){
        $duration = $dataVal;
        $end = $end;
        $split_id = $splitRef;
      ...
      public function findSplit($ref){
        ...
        if (sizeof($output) > 0) {
            ...
            //$data['graph'] = $output;
            //$data['last_updated_by'] = get_last_updated_username($output[0]['last_updated_by']);
...
// get last_updated by username
    ...
    public function findPartTool($dateRef){
        ...
        $query->select('s.time,s.tool_id,s.part_id,p.part_name');
        //$query->where('s.date', $dateRef);
        $query->join('settings_part_current as p', 'p.part_id = s.part_id');
        $output = $query->get()->getResultArray();
        return $output;
    }

    // downtime graph model
    public function findToolTipPart($date,$shift,$machine){
        $db = \Config\Database::connect();
        // $sql = "SELECT `pd.machine_event_id`,`pd.start_time`,`pd.shift_date`,`m.part_id`,`p.part_name` FROM ((`pdm_events` as `pd` LEFT JOIN `pdm_downtime_reason_mapping` as `m` ON `pd.machine_event_id`=`m.machine_event_id`)LEFT JOIN `settings_part_current` as `p` ON `p.part_id`=`m.part_id`)";
        // $result = $db->query($sql);
